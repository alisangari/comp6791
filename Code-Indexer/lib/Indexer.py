...
from lib import InvertedIndex as ii
...
class ReuterIndexer:
    ...
    def addToDictionary(self,index, terms, docId ):
        for term in terms:
        ...
        try:
            ...
            for (newid, article) in zip(regexId.findall(text), regexText.findall(text)):
                ...
                if article: # In case there is no body for this docId
                    ...
                    # for term in terms:
                    #     index[term] = int(docId)
                    self.addToDictionary(index, terms, docId )
                print "Print index: ", index
        finally:
            print "  Tokenising Completed."
            f.close()
    ...
    def display(self, docId, folder='reuters21578'):
        try:
            ...
            # print body
            QueryFile = open("Query_results_documets.txt", "ab+")
            QueryFile.write("Document ID: "+ str(docId)+"\n")
            QueryFile.write(title)
            QueryFile.write("\n")
            QueryFile.write(body)
            QueryFile.write("\n")
            QueryFile.close()
        finally:
            f.close()
